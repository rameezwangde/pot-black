...
export default function AdminBookingsPage() {
  ...
  return <section className="mx-auto max-w-[1550px]">
    ...
    {error && !bookings.length ? <div className="border border-red-800/40 p-10 text-center"><p className="text-sm text-red-200">{error}</p><button type="button" onClick={() => void loadBookings()} className="mt-5 bg-[#D4AF37] px-6 py-3 text-[9px] uppercase text-black">Retry</button></div> : loading ? <BookingListSkeleton/> : !bookings.length ? <EmptyState icon={hasActiveFilters ? SearchX : CalendarPlus2} title={hasActiveFilters ? 'No Matching Bookings' : 'No Bookings Yet'} description={hasActiveFilters ? 'No reservation matches the selected filters. Clear them to return to the full booking list.' : 'New reservations and walk-ins will appear here as soon as they are created.'} actionLabel={hasActiveFilters ? 'Clear Filters' : 'Add Walk-In Booking'} onAction={hasActiveFilters ? clearFilters : () => setWalkInOpen(true)}/> : <>
