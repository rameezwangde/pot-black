...
export default function AdminBookingsPage() {
  ...
  useEffect(() => { getTables().then(data => setTables(data.filter(table => table.isActive && table.status === 'active'))).catch(() => undefined); }, []);
  ...
  const loadDetails = useCallback(async (identifier: string) => {
    setDetailsLoading(true);
    try { setDetails(await getAdminBooking(identifier)); }
    catch (requestError) {
      showToast((requestError as AdminApiError).message, 'error');
      setSearchParams(current => { const next = new URLSearchParams(current); next.delete('booking'); return next; }, { replace: true });
    }
  ...
  const refreshAfterAction = async (message: string) => {
    showToast(message); await loadBookings();
    if (bookingIdentifier) await loadDetails(bookingIdentifier);
  };
