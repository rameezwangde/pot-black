...
export default function AdminWalkInsPage(){
  ...
  useEffect(()=>{getTables().then(data=>setTables(data.filter(table=>table.isActive&&table.status==='active'))).catch(()=>undefined).finally(()=>setTablesLoading(false))},[]);
  ...
  const refresh=async(message:string)=>{showToast(message);await load(true);if(details){try{setDetails(await getAdminBooking(details.bookingReference))}catch{setDetails(undefined)}}};
