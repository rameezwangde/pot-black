...
export default function AdminWalkInsPage(){
  ...
  const changeStatus=async(booking:AdminBooking,nextStatus:BookingStatus)=>{if(actionPending.current)return;actionPending.current=true;setActionLoading(`${booking._id}:${nextStatus}`);try{await updateBookingStatus(booking.bookingReference,nextStatus);await refresh('Booking status updated successfully.')}catch(requestError){showToast((requestError as AdminApiError).message,'error')}finally{actionPending.current=false;setActionLoading('')}};
  ...
  const actions=(booking:AdminBooking)=><div className="flex flex-wrap gap-2"><button type="button" onClick={()=>void openDetails(booking)} className={actionClass}>View</button>{statusTransitions[booking.status].filter(next=>!['cancelled','no-show','confirmed'].includes(next)).map(next=><button key={next} type="button" disabled={Boolean(actionLoading)} data-loading={actionLoading===`${booking._id}:${next}`} aria-busy={actionLoading===`${booking._id}:${next}`} onClick={()=>void changeStatus(booking,next)} className={actionClass}>{actionLoading===`${booking._id}:${next}`?'Updating...':actionLabels[next]??statusLabels[next]}</button>)}{booking.status==='playing'&&<button type="button" onClick={()=>setExtendTarget(booking)} className={actionClass}>Extend</button>}{['confirmed','checked-in','playing'].includes(booking.status)&&<button type="button" onClick={()=>setMoveTarget(booking)} className={actionClass}>Move Table</button>}{['confirmed','checked-in'].includes(booking.status)&&<button type="button" onClick={()=>setCancelTarget(booking)} className={`${actionClass} border-red-900/40 text-red-300`}>Cancel</button>}</div>;
