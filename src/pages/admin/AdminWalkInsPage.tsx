...
export default function AdminWalkInsPage(){
  ...
  const load=useCallback(async(background=false)=>{if(!background)setLoading(true);setError('');try{const result=await getAdminBookings({date,status:status||undefined,tableId:tableId||undefined,search:search||undefined,source:'walk-in',limit:100,sort:'start-asc'});setBookings(result.bookings);setTotal(result.pagination.total)}catch(requestError){if(!background)setError((requestError as AdminApiError).message)}finally{if(!background)setLoading(false)}},[date,status,tableId,search]);
  ...
  const refresh=async(message:string)=>{showToast(message);await load();if(details){try{setDetails(await getAdminBooking(details.bookingReference))}catch{setDetails(undefined)}}};
  ...
  const actions=(booking:AdminBooking)=><div className="flex flex-wrap gap-2"><button type="button" onClick={()=>void openDetails(booking)} className={actionClass}>View</button>{statusTransitions[booking.status].filter(next=>!['cancelled','no-show','confirmed'].includes(next)).map(next=><button key={next} type="button" disabled={Boolean(actionLoading)} onClick={()=>void changeStatus(booking,next)} className={actionClass}>{actionLoading===`${booking._id}:${next}`?'Updating...':actionLabels[next]??statusLabels[next]}</button>)}{booking.status==='playing'&&<button type="button" onClick={()=>setExtendTarget(booking)} className={actionClass}>Extend</button>}{['confirmed','checked-in','playing'].includes(booking.status)&&<button type="button" onClick={()=>setMoveTarget(booking)} className={actionClass}>Move Table</button>}{['confirmed','checked-in'].includes(booking.status)&&<button type="button" onClick={()=>setCancelTarget(booking)} className={`${actionClass} border-red-900/40 text-red-300`}>Cancel</button>}</div>;
  ...
  return <section className="mx-auto max-w-[1500px]">
    <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-2 text-[9px] uppercase tracking-[.3em] text-[#D4AF37]">On-Site Guests</p><h2 className="mb-3 text-4xl text-[#F3E5AB] sm:text-5xl">Walk-In Management</h2><p className="text-sm text-gray-400">Register on-site guests quickly and monitor today’s walk-in reservations.</p></div><button type="button" onClick={()=>setWalkInOpen(true)} className="flex items-center justify-center gap-2 bg-[#D4AF37] px-5 py-3.5 text-[9px] font-semibold uppercase text-black"><Plus size={15}/>New Walk-In Booking</button></div>
    ...
    <div className="mb-6 grid gap-3 border border-[#D4AF37]/15 bg-[#140a0b]/60 p-4 sm:grid-cols-2 xl:grid-cols-4"><input type="date" value={date} onChange={event=>setDate(event.target.value)} className={field}/><select value={status} onChange={event=>setStatus(event.target.value)} className={field}><option value="">All Statuses</option>{Object.entries(statusLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select value={tableId} onChange={event=>setTableId(event.target.value)} className={field}><option value="">All Tables</option>{tables.map(table=><option key={table._id} value={table._id}>{table.code} · {table.name}</option>)}</select><label className="relative"><Search size={14} className="absolute left-3 top-3.5 text-gray-600"/><input value={searchInput} onChange={event=>setSearchInput(event.target.value)} placeholder="Search reference, customer, phone or email" className={`${field} w-full pl-9`}/></label></div>
    {error?<div className="border border-red-900/40 p-10 text-center"><p className="text-red-200">{error}</p><button type="button" onClick={()=>void load()} className="mt-4 bg-[#D4AF37] px-5 py-3 text-[9px] text-black">Retry</button></div>:loading?<div className="space-y-3">{Array.from({length:5}).map((_,index)=><div key={index} className="h-20 animate-pulse bg-white/[.025]"/>)}</div>:!bookings.length?<div className="border border-white/10 p-14 text-center text-sm text-gray-500">No walk-in bookings found.</div>:<><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left"><thead><tr className="border-b border-[#D4AF37]/20 text-[8px] uppercase text-gray-500">{['Reference','Customer','Phone','Table','Time','Players','Status','Actions'].map(label=><th key={label} className="px-3 py-4 font-normal">{label}</th>)}</tr></thead><tbody className="divide-y divide-white/5">{bookings.map(booking=><tr key={booking._id} className="bg-[#140a0b]/40 text-xs"><td className="px-3 py-4 text-[#D4AF37]">{booking.bookingReference}</td><td className="px-3 py-4 text-[#F3E5AB]">{booking.customerName}</td><td className="px-3 py-4 text-gray-400">{booking.phone}</td><td className="px-3 py-4 text-gray-300">{booking.table.code} · {booking.table.name}</td><td className="px-3 py-4 text-gray-300">{formatAdminDate(booking.startDateTime)}<p className="mt-1 text-[9px] text-gray-500">{formatTimeRange(booking)}</p></td><td className="px-3 py-4">{booking.players}</td><td className="px-3 py-4"><BookingStatusBadge status={booking.status}/></td><td className="px-3 py-4">{actions(booking)}</td></tr>)}</tbody></table></div><div className="grid gap-4 md:hidden">{bookings.map(booking=><article key={booking._id} className="border border-white/10 bg-[#140a0b]/70 p-5"><div className="flex justify-between gap-3"><div><p className="text-[9px] text-[#D4AF37]">{booking.bookingReference}</p><h3 className="mt-1 text-xl text-[#F3E5AB]">{booking.customerName}</h3><p className="text-[10px] text-gray-500">{booking.phone}</p></div><BookingStatusBadge status={booking.status}/></div><div className="my-4 grid grid-cols-2 gap-3 border-y border-white/5 py-4 text-xs"><p>{booking.table.name} ({booking.table.code})</p><p>{booking.players} players</p><p>{formatAdminDate(booking.startDateTime)}</p><p>{formatTimeRange(booking)}</p></div>{actions(booking)}</article>)}</div></>}
    {details&&<BookingDetailsPanel booking={details} loading={detailsLoading} onClose={()=>setDetails(undefined)} onStatus={next=>void changeStatus(details,next)} onExtend={()=>setExtendTarget(details)} onCancel={()=>setCancelTarget(details)} onMove={()=>setMoveTarget(details)}/>} 
