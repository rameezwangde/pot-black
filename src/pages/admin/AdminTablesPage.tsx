...
export default function AdminTablesPage() {
  ...
  return <section className="mx-auto max-w-[1550px]">
    <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-2 text-[9px] uppercase tracking-[.3em] text-[#D4AF37]">Live Floor Operations</p><h2 className="mb-3 text-4xl text-[#F3E5AB] sm:text-5xl">Table Management</h2><p className="max-w-2xl text-sm text-gray-400">Monitor live table usage, update availability and manage the tables offered for reservation.</p></div><div className="flex gap-3"><button type="button" onClick={() => void load(true)} className="flex items-center gap-2 border border-[#D4AF37]/30 px-4 py-3 text-[9px] uppercase text-[#F3E5AB]"><RefreshCw size={14} className={refreshing ? 'animate-spin' : ''}/>Refresh</button><button type="button" onClick={() => setFormTable(null)} className="flex items-center gap-2 bg-[#D4AF37] px-5 py-3 text-[9px] font-semibold uppercase text-black"><Plus size={15}/>Add Table</button></div></div>
