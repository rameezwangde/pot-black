...
export default function AdminLayout() {
  ...
  const isActivePath = (href: string) => location.pathname === href || (href !== '/admin' && location.pathname.startsWith(`${href}/`));
  const handleLogout = () => { setIsDrawerOpen(false); logout(); };
  ...
  const sidebar = <div className="flex h-full flex-col bg-[#140a0b]">
    <div className="flex h-24 items-center justify-between border-b border-white/10 px-6"><Link to="/admin" onClick={() => setIsDrawerOpen(false)} className="flex items-center gap-3 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]"><img src="/logo.png" alt="Pot Black" className="h-12 w-auto"/><span className="text-[9px] uppercase tracking-[.24em] text-[#D4AF37]">Operations Panel</span></Link><button type="button" onClick={() => setIsDrawerOpen(false)} aria-label="Close navigation" className="text-gray-500 hover:text-[#D4AF37] focus:outline-none focus:ring-1 focus:ring-[#D4AF37] lg:hidden"><X size={22}/></button></div>
    <nav aria-label="Admin navigation" className="flex-1 space-y-2 px-4 py-8">{navItems.map(item => { const Icon = item.icon; const isActive = isActivePath(item.href!); return item.enabled ? <Link key={item.label} to={item.href!} onClick={() => setIsDrawerOpen(false)} aria-current={isActive ? 'page' : undefined} className={`flex items-center gap-3 border px-4 py-3.5 text-xs uppercase tracking-[.13em] focus:outline-none focus:ring-1 focus:ring-[#D4AF37] ${isActive ? 'border-[#D4AF37]/30 bg-[#D4AF37]/10 text-[#F3E5AB]' : 'border-transparent text-gray-500 hover:border-white/10 hover:text-[#F3E5AB]'}`}><Icon size={17} className={isActive ? 'text-[#D4AF37]' : undefined}/>{item.label}</Link> : <button key={item.label} type="button" disabled className="flex w-full items-center gap-3 border border-transparent px-4 py-3.5 text-left text-xs uppercase tracking-[.13em] text-gray-600"><Icon size={17}/><span>{item.label}</span><span className="ml-auto text-[7px] tracking-[.12em]">Coming Soon</span></button>; })}</nav>
