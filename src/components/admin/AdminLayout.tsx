...
const navItems = [
  ...
  { label: 'Bookings', icon: CalendarDays, enabled: false },
...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#080605] text-white">
    ...
    <div className="lg:pl-72">
      <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-white/10 bg-[#0d0807]/95 px-4 backdrop-blur-xl sm:px-7 lg:px-10"><div className="flex min-w-0 items-center gap-4"><button type="button" onClick={() => setIsDrawerOpen(true)} aria-label="Open navigation" aria-expanded={isDrawerOpen} className="shrink-0 text-[#F3E5AB] focus:outline-none focus:ring-1 focus:ring-[#D4AF37] lg:hidden"><Menu size={24}/></button><div><p className="text-[8px] uppercase tracking-[.22em] text-[#D4AF37]">Operations Panel</p><h1 className="truncate text-xl text-[#F3E5AB] sm:text-2xl">Overview</h1></div></div><div className="flex items-center gap-3 sm:gap-5"><div className="hidden text-right sm:block"><p className="text-[9px] uppercase tracking-[.14em] text-gray-500">Dubai</p><p className="mt-1 text-xs text-gray-300">{currentDate}</p></div><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#D4AF37]/30 bg-[#D4AF37]/10 text-xs text-[#F3E5AB]">{initials}</span></div></header>
