...
import ApiErrorCard from '../common/ApiErrorCard';

export default function ProtectedAdminRoute({ children }: { children: ReactNode }) {
  const { isAuthenticated, isAuthLoading, authError, refreshAdmin } = useAdminAuth();
  ...
  if (authError) return <div className="flex min-h-screen items-center justify-center bg-[#080605] px-4"><div className="w-full max-w-xl"><ApiErrorCard title="Operations Service Unavailable" message={authError} onRetry={() => void refreshAdmin()}/></div></div>;
