...
export default function WalkInBookingModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: (bookingReference?: string) => void }) {
  ...
  useEffect(() => { getTables().then(data => setTables(data.filter(table => table.isActive && table.status === 'active'))).catch(requestError => setError((requestError as AdminApiError).message)); }, []);
