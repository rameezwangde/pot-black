...
export default function BookingDetailsPanel({ booking, loading, onClose, onStatus, onExtend, onCancel, onMove, updatingStatus }: { booking?: AdminBooking; loading: boolean; onClose: () => void; onStatus: (status: BookingStatus) => void; onExtend: () => void; onCancel: () => void; onMove: () => void; updatingStatus?: BookingStatus }) {
  ...
  return <div className="fixed inset-0 z-[90] bg-black/70 backdrop-blur-sm" onMouseDown={event => event.target === event.currentTarget && onClose()}><aside role="dialog" aria-modal="true" aria-label="Booking details" className="absolute inset-y-0 right-0 w-full overflow-y-auto border-l border-[#D4AF37]/20 bg-[#12090a] shadow-2xl sm:max-w-xl">
    <header className="sticky top-0 z-10 flex items-start justify-between gap-5 border-b border-white/10 bg-[#12090a]/95 p-5 backdrop-blur sm:p-7"><div><p className="text-[9px] uppercase tracking-[.2em] text-[#D4AF37]">Booking Details</p><h2 className="mt-2 break-all text-xl sm:text-2xl text-[#F3E5AB]">{booking?.bookingReference ?? 'Loading...'}</h2></div><button type="button" onClick={onClose} aria-label="Close booking details" className="text-gray-500 hover:text-[#D4AF37]"><X size={23}/></button></header>
