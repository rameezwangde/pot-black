...
export default function TableStatusModal({ table, action, onClose, onSuccess }: { table: OperationalTable; action: TableStatusAction; onClose: () => void; onSuccess: (message: string) => void }) {
  ...
  return <AdminModalShell title={labels[action]} onClose={onClose}>
    ...
    <div className="mt-6 flex gap-3"><button type="button" onClick={onClose} className="flex-1 border border-white/10 py-3 text-[9px] uppercase text-gray-400">Close</button><button type="button" onClick={() => void submit()} disabled={submitting || (hasFuture && isRestrictiveStatus && !force)} aria-busy={submitting} className={`flex-1 py-3 text-[9px] font-semibold uppercase disabled:opacity-40 ${action === 'deactivate' ? 'bg-[#6f171a] text-white' : 'bg-[#D4AF37] text-black'}`}><InlineLoadingLabel loading={submitting} loadingText="Updating...">{labels[action]}</InlineLoadingLabel></button></div>
  </AdminModalShell>;
}
