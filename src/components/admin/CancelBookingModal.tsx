...
export default function CancelBookingModal({ booking, onClose, onSuccess }: { booking: AdminBooking; onClose: () => void; onSuccess: () => void }) {
  ...
  return <AdminModalShell title="Cancel Booking" onClose={onClose}>
    ...
    <div className="mt-6 flex gap-3"><button type="button" onClick={onClose} className="flex-1 border border-white/10 py-3 text-[9px] uppercase tracking-[.16em] text-gray-400">Keep Booking</button><button type="button" onClick={() => void submit()} disabled={submitting} className="flex-1 bg-[#6f171a] py-3 text-[9px] font-semibold uppercase tracking-[.16em] text-white disabled:opacity-60">{submitting ? 'Cancelling...' : 'Cancel Booking'}</button></div>
  </AdminModalShell>;
}
