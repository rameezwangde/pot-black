...
export default function CancelBookingModal({ booking, onClose, onSuccess }: { booking: AdminBooking; onClose: () => void; onSuccess: () => void }) {
  ...
  return <AdminModalShell title="Cancel Booking" onClose={close}>
    ...
    <div className="mb-4 flex flex-wrap gap-2">{reasons.map(item => <button type="button" key={item} disabled={submitting} onClick={() => setReason(item === 'Other' ? '' : item)} className="border border-white/10 px-3 py-2 text-[9px] text-gray-400 hover:border-[#D4AF37]/40 hover:text-[#F3E5AB]">{item}</button>)}</div>
    ...
    <div className="mt-6 flex gap-3"><button type="button" onClick={close} disabled={submitting} className="flex-1 border border-white/10 py-3 text-[9px] uppercase tracking-[.16em] text-gray-400">Keep Booking</button><button type="button" onClick={() => void submit()} disabled={submitting} aria-busy={submitting} className="flex-1 bg-[#6f171a] py-3 text-[9px] font-semibold uppercase tracking-[.16em] text-white disabled:opacity-60"><InlineLoadingLabel loading={submitting} loadingText="Cancelling...">Cancel Booking</InlineLoadingLabel></button></div>
  </AdminModalShell>;
}
