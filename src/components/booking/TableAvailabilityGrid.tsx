...
export default function TableAvailabilityGrid({ tables, selectedId, loading, onSelect }: { tables: BookingTable[]; selectedId?: string; loading: boolean; onSelect: (table: BookingTable) => void }) {
  ...
  if (!tables.length) return <div className="border border-[#D4AF37]/15 bg-[#100b0a] p-6 text-center sm:p-12 text-gray-400">No tables are currently available for the selected date and group size.</div>;
