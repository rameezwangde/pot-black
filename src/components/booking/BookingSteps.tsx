...
export default function BookingSteps({ activeStep }: { activeStep: number }) {
  return (
    <div aria-label="Booking progress" className="overflow-x-auto pb-2">
      <ol className="grid grid-cols-4 min-w-[650px] border border-[#D4AF37]/15 bg-[#100b0a]/90">
        {steps.map((step, index) => {
          const current = index + 1;
          const active = current <= activeStep;
          return <li key={step} aria-current={current === activeStep ? 'step' : undefined} className="relative flex items-center gap-3 px-5 py-5 border-r last:border-r-0 border-[#D4AF37]/15">
            ...
            <span className={`text-[10px] uppercase tracking-[.18em] ${active ? 'text-[#F3E5AB]' : 'text-gray-500'}`}>{step}</span>
