...
export default function BookingSuccessModal({ open, booking, onClose, onReset }: { open: boolean; booking?: CreatedBooking; onClose: () => void; onReset: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] bg-black/85 backdrop-blur-sm p-4 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="success-title" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <div className="relative w-full max-w-xl border border-[#D4AF37]/35 bg-[#100b0a] p-6 sm:p-10 shadow-[0_0_80px_rgba(0,0,0,.8)]"><button onClick={onClose} aria-label="Close confirmation" className="absolute top-4 right-4 text-gray-500 hover:text-[#D4AF37] focus:outline-none focus:ring-2 focus:ring-[#D4AF37]"><X /></button><CheckCircle2 className="text-[#D4AF37] mb-5" size={38} strokeWidth={1.2}/><p className="text-[9px] uppercase tracking-[.3em] text-[#D4AF37] mb-2">Booking Reference: {booking.bookingReference}</p><h2 id="success-title" className="text-3xl sm:text-4xl text-[#F3E5AB] mb-4">Reservation Confirmed</h2><p className="text-sm text-gray-400 leading-relaxed mb-7">Your Pot Black table is confirmed. Please keep your booking reference for check-in.</p>
